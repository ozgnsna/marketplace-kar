...
interface TargetPriceCardProps {
  ...
  emphasize?: boolean;
  /** Alış maliyeti girilmeden öneri göstermez */
  hasCost?: boolean;
}
...
export function TargetPriceCard({
  ...
  const solved = useMemo(() => {
    if (!hasCost || inputs.purchasePrice <= 0) return null;
  ...
  }, [hasCost, inputs, targetMode, targetNet, marginOnCost]);
  ...
  return (
    ...
    >
      ...
      <h3
        className={`text-lg font-bold text-[#0B1F3B] ${emphasize && hasCost ? "pr-24 sm:pr-32" : ""}`}
      >
      ...
      {!hasCost ? (
        <p
          className="mt-5 rounded-2xl border border-dashed border-slate-200 bg-slate-50/90 px-4 py-8 text-center text-sm leading-relaxed text-slate-600"
          role="status"
        >
          Önce ürün maliyetini girin; hedef satış fiyatı burada görünecek.
        </p>
      ) : null}
